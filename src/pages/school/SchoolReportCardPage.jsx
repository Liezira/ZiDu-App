...
import {
  ...
  ClipboardCheck, MessageSquare, AlertCircle, Printer,
} from 'lucide-react';
...
// ── PDF generator (same as teacher version) ───────────────────
const generatePDF = (data, comment, periode) => {
  ...
  <div class="header">
    ...
    <h2>LAPORAN HASIL BELAJAR SISWA (LENGKAP)</h2>
  ...
  <div class="section-title">NILAI UJIAN PER MATA PELAJARAN (SEMUA GURU)</div>
  ${Object.keys(bySubject).length === 0
    ? '<p style="color:#94A3B8;font-size:12px;padding:12px 0">Belum ada nilai ujian dalam periode ini.</p>'
    : `<table>
      <thead><tr>
        <th class="left">Mata Pelajaran</th><th>Kode</th>
  ...
  <div class="att-grid">
    <div class="att-card" style="background:#F0FDF4"><div class="att-num" style="color:#16A34A">${attendance?.total_hadir || 0}</div><div class="att-lbl" style="color:#16A34A">HADIR</div></div>
    <div class="att-card" style="background:#EFF6FF"><div class="att-num" style="color:#0891B2">${attendance?.total_izin || 0}</div><div class="att-lbl" style="color:#0891B2">IZIN</div></div>
    <div class="att-card" style="background:#FFFBEB"><div class="att-num" style="color:#D97706">${attendance?.total_sakit || 0}</div><div class="att-lbl" style="color:#D97706">SAKIT</div></div>
    <div class="att-card" style="background:#FEF2F2"><div class="att-num" style="color:#DC2626">${attendance?.total_alpha || 0}</div><div class="att-lbl" style="color:#DC2626">ALPHA</div></div>
    <div class="att-card" style="background:#F8FAFC"><div class="att-num" style="color:#475569">${attendance?.total_sesi || 0}</div><div class="att-lbl" style="color:#475569">TOTAL SESI</div></div>
  </div>
  <div class="rank-box">
    <div>
  ...
  ${comment ? `<div class="section-title">CATATAN</div><div style="background:#FFFBEB;border:1px solid #FDE68A;border-radius:10px;padding:14px 16px;margin-bottom:20px"><p style="font-style:italic;color:#92400E;line-height:1.7">"${comment}"</p></div>` : ''}
  <div class="footer"><div class="sign-box"><div style="font-size:12px;color:#64748B">Dicetak: ${fmtFull(todayISO())}</div><div class="sign-line">Kepala Sekolah / Admin</div></div></div>
  <script>window.onload = () => window.print();</script>
  </body></html>`);
  win.document.close();
};

// ── Report Drawer (tampilkan semua mapel tanpa filter guru) ───
const ReportDrawer = ({ student, dateFrom, dateTo, periode, profile, onClose }) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const { data: rcData, error } = await supabase.rpc('get_report_card', {
        p_student_id: student.id,
        p_date_from:  dateFrom,
        p_date_to:    dateTo,
        p_teacher_id: null,   // null = semua guru (admin sekolah)
      });
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', justifyContent: 'flex-end' }}>
      ...
      <div style={{ position: 'relative', width: '100%', maxWidth: 560, height: '100%', background: '#fff', display: 'flex', flexDirection: 'column', boxShadow: '-8px 0 32px rgba(0,0,0,.12)', animation: 'slideLeft .22s ease', overflow: 'hidden' }}>

        {/* Header */}
        <div style={{ padding: '18px 22px', borderBottom: '1px solid #F1F5F9', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', background: 'linear-gradient(135deg,#0F4C81,#0284C7)', color: '#fff' }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, opacity: .7, letterSpacing: '.08em', marginBottom: 4 }}>RAPOR LENGKAP · SEMUA MATA PELAJARAN</div>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <button
              onClick={() => data && generatePDF(data, '', periode)}
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          {!loading && data && <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
              {[
                { label: 'Rata-rata', value: rnk.avg_score || 0, color: (rnk.avg_score || 0) >= 75 ? '#16A34A' : '#DC2626', bg: '#F0FDF4', icon: TrendingUp },
                { label: 'Ranking',   value: `${rnk.rank || '—'}/${rnk.total_siswa || '—'}`, color: '#2563EB', bg: '#EFF6FF', icon: Award },
            ...
            {/* Nilai per mapel — semua guru */}
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#475569', letterSpacing: '.06em', marginBottom: 10 }}>📚 NILAI PER MATA PELAJARAN (SEMUA GURU)</div>
              {Object.keys(bySubject).length === 0 ? (
...
export default function SchoolReportCardPage() {
  ...
  }, [classId, allClasses, profile?.school_id]);

  const filtered = students.filter(s =>
    !search || s.name.toLowerCase().includes(search.toLowerCase()) || (s.nis || '').includes(search)
  );

  // ── Batch export (semua siswa di kelas, tanpa filter guru) ──
    ...
    try {
      for (let i = 0; i < filtered.length; i++) {
        const { data } = await supabase.rpc('get_report_card', {
          ...
          p_date_from:  dateFrom,
          p_date_to:    dateTo,
          p_teacher_id: null,   // admin: lihat semua guru
        });
        if (data) generatePDF(data, '', periode);
  ...
  return (
    <>
      <style>{`
        ...
        .stu-row:hover { background:#EFF6FF !important; cursor:pointer }
      `}</style>
      ...
      <div style={{ fontFamily: "'DM Sans',sans-serif", display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div style={{ opacity: 0, animation: 'fadeUp .4s ease forwards', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
              ...
              <h1 style={{ fontFamily: 'Sora,sans-serif', fontSize: 22, fontWeight: 700, color: '#0F172A', margin: 0 }}>Rapor Siswa</h1>
            </div>
            <p style={{ fontSize: 14, color: '#64748B', margin: 0 }}>Laporan lengkap nilai semua mata pelajaran per siswa</p>
          </div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            <button
              ...
              disabled={batchLoading || !filtered.length}
              ...
              {batchLoading
                ? <><div style={{ width: 12, height: 12, borderRadius: '50%', border: '2px solid #E2E8F0', borderTopColor: '#0284C7', animation: 'spin .7s linear infinite' }} />Mengexport…</>
        ...
        {error && (
          <div style={{ padding: 14, background: '#FEF2F2', borderRadius: 12, color: '#DC2626', fontSize: 13, display: 'flex', gap: 8 }}>
          ...
          <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #F1F5F9', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,.03)' }}>
            <div style={{ padding: '12px 18px', borderBottom: '1px solid #F1F5F9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: 13, fontWeight: 600, color: '#475569' }}>
                <strong style={{ color: '#0F172A' }}>{filtered.length}</strong> siswa · {allClasses.find(c => c.id === classId)?.name}
              </span>
              <span style={{ fontSize: 12, color: '#94A3B8' }}>Klik siswa untuk lihat rapor lengkap</span>
            </div>
                  ...
                  <tr key={s.id} className="stu-row" onClick={() => setActiveStudent(s)}
                    ...
                    <td style={{ padding: '13px 16px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'linear-gradient(135deg,#0284C7,#0EA5E9)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 12, fontWeight: 700, flexShrink: 0 }}>
                    ...
                    <td style={{ padding: '13px 16px', textAlign: 'center' }} onClick={e => e.stopPropagation()}>
                      <button onClick={() => setActiveStudent(s)}
                        style={{ padding: '5px 14px', borderRadius: 7, border: '1px solid #E2E8F0', background: '#F8FAFC', fontSize: 12, fontWeight: 600, color: '#0284C7', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
