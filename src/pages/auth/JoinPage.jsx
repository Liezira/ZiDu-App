    ...
    <div style={{ marginBottom: 4 }}>
      <div style={{ display: 'flex', gap: 8, marginBottom: customSubjects.length > 0 ? 10 : 0 }}>
        <div style={{ flex: 1, position: 'relative' }}>
          ...
          <input type="text" value={val} onChange={e => setVal(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); submit(); } }} onFocus={() => setFocused(true)} onBlur={() => setFocused(false)} placeholder="Ketik nama mapel, lalu Enter..."
            style={{ width: '100%', padding: '10px 14px 10px 40px', borderRadius: 10, fontSize: 13.5, border: `1.5px solid ${focused ? '#6366F1' : 'rgba(255,255,255,.1)'}`, background: focused ? 'rgba(99,102,241,.06)' : 'rgba(255,255,255,.04)', color: '#F1F5F9', outline: 'none', boxSizing: 'border-box', boxShadow: focused ? '0 0 0 4px rgba(99,102,241,.1)' : 'none', transition: 'all .2s', fontFamily: "'Plus Jakarta Sans',sans-serif" }} />
        </div>
        <button type="button" onClick={submit} disabled={!val.trim()}
          style={{ padding: '10px 16px', borderRadius: 10, border: 'none', background: val.trim() ? 'linear-gradient(135deg,#4F46E5,#6366F1)' : 'rgba(255,255,255,.06)', color: val.trim() ? '#fff' : '#475569', fontSize: 13, fontWeight: 700, cursor: val.trim() ? 'pointer' : 'not-allowed', fontFamily: "'Plus Jakarta Sans',sans-serif", transition: 'all .2s', whiteSpace: 'nowrap', flexShrink: 0, boxShadow: val.trim() ? '0 0 14px rgba(99,102,241,.3)' : 'none' }}>
          + Tambah
        </button>
      </div>
      {customSubjects.length > 0 && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
          {customSubjects.map(s => (
            <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 12px', borderRadius: 9, border: '2px solid #6366F1', background: 'rgba(99,102,241,.14)', color: '#C7D2FE', fontSize: 13, fontWeight: 700, boxShadow: '0 0 12px rgba(99,102,241,.2)' }}>
              <BookOpen size={11} color="#818CF8" />{s.name}
              <button type="button" onClick={() => onRemove(s.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#818CF8', display: 'flex', alignItems: 'center', padding: 0, marginLeft: 2 }}>
...
const JoinPage = () => {
  ...
  const isStudent     = invite?.target_role === 'student';
  const isSchoolAdmin = invite?.target_role === 'school_admin';
  ...
  const addCustomSubject   = n => { const t = n.trim(); if (!t || customSubjects.find(s => s.name.toLowerCase() === t.toLowerCase())) return; setCustomSubjects(p => [...p, { id: `custom_${Date.now()}`, name: t, isCustom: true }]); };
  const removeCustomSubject = id => setCustomSubjects(p => p.filter(s => s.id !== id));

  const validateStep1 = () => {
    const e = {};
    if (!name.trim())  e.name = 'Nama lengkap wajib diisi';
    if (isStudent && !nis.trim()) e.nis = 'NIS wajib diisi untuk siswa';
  ...
  const handleStep1 = async e => {
    ...
    try {
      ...
      await supabase.from('profiles').upsert([{
        ...
        nis: isStudent ? nis.trim() : null,
        status: 'active',
        created_at: new Date().toISOString(), updated_at: new Date().toISOString(),
      }]);
      await useInviteLink(token, uid);
      if (isTeacher) { setNewUserId(uid); setStep(2); }
  ...
  const handleStep2 = async () => {
    ...
    try {
      ...
      if (customSubjects.length > 0) {
        const makeCode = n => n.replace(/[^A-Za-z0-9]/g, '').toUpperCase().slice(0, 8) || 'MAPEL';
        const inserts = customSubjects.map(s => ({ school_id: invite.school_id, name: s.name, code: makeCode(s.name), created_at: new Date().toISOString(), updated_at: new Date().toISOString() }));
        const { data: newSubjs, error: subjErr } = await supabase.from('subjects').insert(inserts).select('id');
        if (subjErr) customNames = customSubjects.map(s => s.name);
        else finalSubjectIds = [...finalSubjectIds, ...(newSubjs || []).map(s => s.id)];
      }
      await supabase.from('profiles').update({ subject_ids: finalSubjectIds, ...(customNames.length > 0 ? { custom_subject_names: customNames } : {}), updated_at: new Date().toISOString() }).eq('id', newUserId);
      setDone(true);
  ...
  const INPUT = fname => ({
    width: '100%', padding: '13px 16px 13px 44px', borderRadius: '12px', fontSize: '14px',
    border: `1.5px solid ${errors[fname] ? '#FECACA' : focused === fname ? '#6366F1' : 'rgba(255,255,255,.1)'}`,
    background: focused === fname ? 'rgba(99,102,241,.06)' : 'rgba(255,255,255,.04)',
    color: '#F1F5F9', outline: 'none',
    boxShadow: focused === fname ? '0 0 0 4px rgba(99,102,241,.12)' : 'none',
    transition: 'all .2s', boxSizing: 'border-box',
    fontFamily: "'Plus Jakarta Sans', sans-serif",
  });

  const ICON_STYLE = fname => ({
    position: 'absolute', left: 15, top: '50%', transform: 'translateY(-50%)',
    pointerEvents: 'none', transition: 'color .2s',
    color: focused === fname ? '#818CF8' : 'rgba(148,163,184,.5)',
  });
  ...
  const formMeta = {
    student:      { title: 'Buat Akun Siswa',        desc: 'Isi data untuk bergabung ke kelas' },
    teacher:      { title: 'Buat Akun Guru',          desc: 'Lengkapi data dan setup jadwal mengajar' },
    school_admin: { title: 'Buat Akun Admin Sekolah', desc: 'Isi data untuk mulai mengelola sekolah' },
  }[invite?.target_role] || { title: 'Buat Akun', desc: 'Isi data untuk bergabung' };
  ...
  if (inviteError) return (
    ...
    <div style={{ minHeight: '100vh', background: '#070B14', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, fontFamily: "'Plus Jakarta Sans',sans-serif" }}>
      <div className="join-in" style={{ width: '100%', maxWidth: 400, background: '#0F1420', borderRadius: 24, border: '1px solid rgba(239,68,68,.2)', padding: '48px 36px', textAlign: 'center', boxShadow: '0 0 60px rgba(239,68,68,.06)' }}>
        <div style={{ width: 60, height: 60, borderRadius: '50%', background: 'rgba(239,68,68,.1)', border: '1.5px solid rgba(239,68,68,.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px' }}><AlertCircle size={24} color="#F87171" /></div>
  ...
  if (done) return (
    ...
    <div style={{ minHeight: '100vh', background: '#070B14', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, fontFamily: "'Plus Jakarta Sans',sans-serif" }}>
      <div className="join-in" style={{ width: '100%', maxWidth: 400, background: '#0F1420', borderRadius: 24, border: '1px solid rgba(16,185,129,.2)', padding: '48px 36px', textAlign: 'center', boxShadow: '0 0 60px rgba(16,185,129,.08)' }}>
        <div style={{ width: 64, height: 64, borderRadius: '50%', background: 'rgba(16,185,129,.1)', border: '1.5px solid rgba(16,185,129,.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px' }}><CheckCircle2 size={28} color="#10B981" /></div>
        <h2 style={{ fontFamily: "'Sora',sans-serif", fontSize: 22, fontWeight: 800, color: '#F1F5F9', marginBottom: 10 }}>Selamat Datang! 🎉</h2>
        <p style={{ fontSize: 14, color: '#475569', lineHeight: 1.7 }}>
          Akun kamu sudah aktif sebagai <strong style={{ color: '#818CF8' }}>{meta.label}</strong>.
          {invite?.class_name ? <> Bergabung ke <strong style={{ color: '#818CF8' }}>{invite.class_name}</strong>.</> : null}
  ...
  const accentGrad = {
    student:      'linear-gradient(135deg,#F59E0B,#FBBF24)',
    teacher:      'linear-gradient(135deg,#10B981,#34D399)',
    school_admin: 'linear-gradient(135deg,#4F46E5,#6366F1)',
  }[invite?.target_role] || 'linear-gradient(135deg,#4F46E5,#6366F1)';

  return (
    <><style>{CSS}</style>
    <div style={{ minHeight: '100vh', background: '#070B14', position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '32px 20px', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      {isTeacher && (
        <div className="join-in-d1" style={{ display: 'flex', alignItems: 'center', gap: 0, marginBottom: 24, width: '100%', maxWidth: 480 }}>
          {[{ n: 1, label: 'Informasi Akun' }, { n: 2, label: 'Setup Mengajar' }].map((s, i) => (
            <React.Fragment key={s.n}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1 }}>
                <div style={{ width: 28, height: 28, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, flexShrink: 0, background: step >= s.n ? 'linear-gradient(135deg,#4F46E5,#6366F1)' : 'rgba(255,255,255,.06)', color: step >= s.n ? '#fff' : '#475569', boxShadow: step >= s.n ? '0 0 14px rgba(99,102,241,.4)' : 'none', transition: 'all .4s' }}>
                  {step > s.n ? <CheckCircle2 size={14}/> : s.n}
                </div>
                <span style={{ fontSize: 13, fontWeight: step === s.n ? 600 : 400, color: step >= s.n ? '#C7D2FE' : '#334155', whiteSpace: 'nowrap', transition: 'color .3s' }}>{s.label}</span>
              </div>
              {i < 1 && <div style={{ flex: 1, height: 2, borderRadius: 99, background: step > 1 ? 'linear-gradient(90deg,#4F46E5,#6366F1)' : 'rgba(255,255,255,.06)', margin: '0 8px', transition: 'background .5s' }} />}
            </React.Fragment>
          ))}
        </div>
      )}
      ...
      {/* Invite chip */}
      {step === 1 && (
        <div className="join-in-d1" style={{ width: '100%', maxWidth: 480, marginBottom: 12, background: 'rgba(255,255,255,.03)', border: `1px solid ${meta.border}22`, borderRadius: 16, padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 800, padding: '3px 10px', borderRadius: 999, background: meta.soft, color: meta.accent, border: `1px solid ${meta.border}`, letterSpacing: '0.02em' }}>{meta.label}</span>
            </div>
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
              {invite.class_name && <span style={{ fontSize: 12, color: '#475569', display: 'flex', alignItems: 'center', gap: 4 }}><School size={11} color="#475569"/>{invite.class_name}</span>}
              <span style={{ fontSize: 12, color: '#334155', display: 'flex', alignItems: 'center', gap: 4 }}>
                <Clock size={11} color="#334155"/>Berlaku hingga {new Date(invite.expires_at).toLocaleDateString('id-ID',{day:'numeric',month:'short',year:'numeric'})}
              </span>
            </div>
          </div>
        </div>
      ...
      {step === 1 && (
        <div className="join-in-d2" style={{ width: '100%', maxWidth: 480, background: '#0F1420', borderRadius: 24, border: '1px solid rgba(255,255,255,.07)', boxShadow: '0 32px 80px rgba(0,0,0,.5)', overflow: 'hidden' }}>
          <div style={{ height: 3, background: accentGrad, backgroundSize: '200% 100%', animation: 'shimmer 3s linear infinite' }} />
          <div style={{ padding: '32px 32px 36px' }}>

            {/* Header dengan icon role */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 28 }}>
              <div style={{ width: 44, height: 44, borderRadius: 13, background: meta.soft, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 0 22px ${meta.glow}`, flexShrink: 0 }}>
                <RoleIcon size={20} color={meta.accent} />
              </div>
              <div>
                <h1 style={{ fontFamily: "'Sora',sans-serif", fontSize: 21, fontWeight: 800, color: '#F1F5F9', marginBottom: 3, letterSpacing: '-0.3px' }}>{formMeta.title}</h1>
                <p style={{ fontSize: 13, color: '#475569' }}>{formMeta.desc}</p>
              </div>
            </div>

            {/* Info box khusus Admin Sekolah */}
            {isSchoolAdmin && (
              <div style={{ marginBottom: 20, padding: '13px 16px', borderRadius: 12, background: 'rgba(79,70,229,.08)', border: '1px solid rgba(79,70,229,.25)', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                <Shield size={15} color="#818CF8" style={{ flexShrink: 0, marginTop: 1 }} />
                <p style={{ fontSize: 12.5, color: '#A5B4FC', lineHeight: 1.65, margin: 0 }}>
                  Akun Admin Sekolah memberi akses penuh untuk mengelola kelas, guru, siswa, dan seluruh data sekolah di ZiDu.
                </p>
              </div>
            )}
            ...
            <form onSubmit={handleStep1} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              ...
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <input type="text" placeholder="Nama lengkap kamu" value={name} onChange={e => setName(e.target.value)} onFocus={() => setFocused('name')} onBlur={() => setFocused('')} style={INPUT('name')} />
                </div>
              ...
              {isStudent && (
                <div>
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    <input type="text" placeholder="Nomor Induk Siswa" value={nis} onChange={e => setNis(e.target.value)} onFocus={() => setFocused('nis')} onBlur={() => setFocused('')} style={INPUT('nis')} />
                  </div>
              ...
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <input type="email" placeholder="email@kamu.com" value={email} onChange={e => setEmail(e.target.value)} onFocus={() => setFocused('email')} onBlur={() => setFocused('')} style={INPUT('email')} />
                </div>
              ...
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <input type={showPass ? 'text' : 'password'} placeholder="Min. 8 karakter" value={password} onChange={e => setPassword(e.target.value)} onFocus={() => setFocused('password')} onBlur={() => setFocused('')} style={{ ...INPUT('password'), paddingRight: 48 }} />
                  <button type="button" onClick={() => setShowPass(s => !s)} style={{ position: 'absolute', right: 14, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#475569', display: 'flex', padding: 4 }}>
                ...
                {password.length > 0 && (
                  <div style={{ marginTop: 8 }}>
                    <div style={{ display: 'flex', gap: 4, marginBottom: 5 }}>
                      {[0,1,2,3].map(i => <div key={i} style={{ flex: 1, height: 3, borderRadius: 99, transition: 'background .3s', background: i < strength ? strengthColors[strength-1] : 'rgba(255,255,255,.07)' }} />)}
                    </div>
                    <span style={{ fontSize: 11.5, color: strength > 0 ? strengthColors[strength-1] : '#475569', fontWeight: 500 }}>{strength > 0 ? strengthLabels[strength-1] : ''}</span>
                  </div>
                )}
              ...
              {errors._form && (
                <div style={{ padding: '12px 16px', borderRadius: 12, background: 'rgba(239,68,68,.08)', border: '1px solid rgba(239,68,68,.2)', color: '#FCA5A5', display: 'flex', alignItems: 'flex-start', gap: 10, fontSize: 13.5, lineHeight: 1.55 }}>
                  ...
                  <div>
                    {errors._form}
                    {errors._form.includes('Terlalu banyak') && <div style={{ fontSize: 12, color: '#94A3B8', marginTop: 4 }}>💡 Ini batas keamanan dari Supabase. Coba lagi dalam 1–2 menit.</div>}
                  </div>
                </div>
              )}
              ...
              <button type="submit" disabled={submitting}
                style={{ width: '100%', padding: '14px', marginTop: 4, borderRadius: 14, border: 'none', background: submitting ? 'rgba(99,102,241,.5)' : accentGrad, color: '#fff', fontSize: 14.5, fontWeight: 700, cursor: submitting ? 'not-allowed' : 'pointer', fontFamily: "'Plus Jakarta Sans',sans-serif", display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9, transition: 'all .2s', boxShadow: submitting ? 'none' : `0 0 24px ${meta.glow}` }}>
                {submitting
        ...
        <div className="join-in-d2" style={{ width: '100%', maxWidth: 520, background: '#0F1420', borderRadius: 24, border: '1px solid rgba(255,255,255,.07)', boxShadow: '0 32px 80px rgba(0,0,0,.5)', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '32px 32px 36px' }}>
            <div style={{ marginBottom: 26 }}>
              ...
              <p style={{ fontSize: 13.5, color: '#475569', lineHeight: 1.55, marginLeft: 48 }}>Pilih mapel yang kamu ajar. Admin bisa mengubah ini kapan saja.</p>
            </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                    <label style={{ fontSize: 12, fontWeight: 700, color: '#94A3B8', letterSpacing: '0.08em', textTransform: 'uppercase' }}>Mata Pelajaran <span style={{ color: '#EF4444' }}>*</span></label>
                    {(selSubjects.length + customSubjects.length) > 0 && <span style={{ fontSize: 12, fontWeight: 700, color: meta.accent, background: meta.soft, padding: '3px 10px', borderRadius: 999 }}>✓ {selSubjects.length + customSubjects.length} dipilih</span>}
                  </div>
                  <AddSubjectInput customSubjects={customSubjects} onAdd={addCustomSubject} onRemove={removeCustomSubject} />
                  {subjects.length > 0 && (
                      ...
                      {subjects.map(s => {
                        ...
                        return (
                          <button key={s.id} type="button" onClick={() => toggleSubject(s.id)}
                            ...
                            <BookOpen size={12} color={active ? '#818CF8' : '#475569'}/>{s.name}
                  ...
                  {subjects.length > 0 && <p style={{ fontSize: 12, color: '#334155', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}><span style={{ flex: 1, height: 1, background: 'rgba(255,255,255,.06)', display: 'inline-block' }}/><span>Mapelmu tidak ada? Tambah di atas</span><span style={{ flex: 1, height: 1, background: 'rgba(255,255,255,.06)', display: 'inline-block' }}/></p>}
                </div>

                {classes.length > 0 && (
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                      <label style={{ fontSize: 12, fontWeight: 700, color: '#94A3B8', letterSpacing: '0.08em', textTransform: 'uppercase' }}>Kelas yang Diajar <span style={{ color: '#334155', fontWeight: 400, textTransform: 'none', fontSize: 11, letterSpacing: 0, marginLeft: 6 }}>(opsional)</span></label>
                      {selClasses.length > 0 && <span style={{ fontSize: 12, fontWeight: 700, color: meta.accent, background: meta.soft, padding: '3px 10px', borderRadius: 999 }}>✓ {selClasses.length} kelas</span>}
                    </div>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
                      {classes.map(cls => {
                        const active = selClasses.includes(cls.id);
                        return <button key={cls.id} type="button" onClick={() => toggleClass(cls.id)} style={{ padding: '8px 14px', borderRadius: 9, border: `2px solid ${active ? meta.accent : 'rgba(255,255,255,.08)'}`, background: active ? `${meta.soft}22` : 'rgba(255,255,255,.03)', color: active ? meta.accent : '#64748B', fontSize: 13, fontWeight: active ? 700 : 500, cursor: 'pointer', transition: 'all .18s', fontFamily: "'Plus Jakarta Sans',sans-serif" }}>{cls.name}</button>;
                      })}
                    </div>
                  </div>
                )}
